            ...
            <div className="overflow-x-auto w-full">
              <table className="w-full text-left border-collapse">
                <thead className="bg-slate-50 border-b border-slate-100">
                  <tr>
                    <th className="px-10 py-6 text-xs font-black text-slate-400 uppercase tracking-[0.2em]">Usuário</th>
                    <th className="px-10 py-6 text-xs font-black text-slate-400 uppercase tracking-[0.2em] text-right whitespace-nowrap">Ações de Nível e Exclusão</th>
                  </tr>
                </thead>
                  ...
                  {usuariosFiltrados.length === 0 ? (
                    <tr>
                      <td colSpan={2} className="p-20 text-center text-slate-400 font-medium">
                        <span className="block text-base font-bold text-slate-500">Nenhum usuário encontrado.</span>
                      </td>
                    </tr>
                  ) : (
                      ...
                      <tr key={user.id} className="hover:bg-slate-50/50 transition-colors group">
                        <td className="px-10 py-6">
                          <div className="flex items-center gap-5">
                            ...
                            {/* Avatar um pouco maior para a tela esticada */}
                            <div className="h-14 w-14 rounded-2xl bg-blue-100 text-blue-700 border-2 border-white flex items-center justify-center font-black text-2xl shadow-sm uppercase shrink-0">
                              {user.nome.charAt(0)}
                            </div>
                            <div>
                              <p className="font-bold text-slate-800 text-lg tracking-tight leading-tight mb-1">{user.nome}</p>
                              <p className="text-sm text-slate-500 flex items-center gap-2 font-medium italic">
                                <Mail size={14} className="text-slate-400" /> {user.email}
                              </p>
                              
                              <span className={`inline-block mt-3 px-3 py-1.5 rounded-lg text-[10px] font-black border uppercase tracking-[0.1em] ${getBadgeColor(user.regra)}`}>
                                {user.regra}
                              </span>
                            </div>
                          </div>
                          ...
                          <div className="flex justify-end items-center gap-5">
                            ...
                            <div className="relative">
                              <select 
                                ...
                                className={`appearance-none border py-3 pl-5 pr-12 rounded-xl text-xs font-black tracking-wide focus:outline-none focus:ring-4 cursor-pointer disabled:opacity-50 transition-all shadow-sm ${getSelectColor(user.regra)}`}
                              >
